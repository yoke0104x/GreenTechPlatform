...
export function MobileContactUsModal({
  ...
  // 关闭弹窗时重置状态（提交后会尝试拉起订阅面板，此时不要立即清理）
  useEffect(() => {
    if (isOpen) return
  ...
  }, [openTagReady, subscribeTemplateId, locale, toast])
  ...
  const requestSubscribe = () => {
    if (!subscribeTemplateId || !openTagReady) {
      toast({
        title: locale === 'en' ? 'Preparing…' : '准备中…',
        description: prepareError || (locale === 'en' ? 'WeChat capability is not ready yet.' : '微信能力尚未就绪'),
        variant: prepareError ? 'destructive' : 'default',
      })
      if (prepareError) setPrepareNonce((v) => v + 1)
      return
    }

    const el: any = subscribeTagRef.current
    if (!el?.click) {
      toast({
        title: locale === 'en' ? 'Failed' : '操作失败',
        description: locale === 'en' ? 'Subscribe button is not ready.' : '订阅按钮未就绪',
        variant: 'destructive',
      })
      return
    }
    ...
    // 某些环境会拦截“非用户触发”的弹窗：给出一个 Toast 让用户再点一次（这次是用户手势）
    if (!toastShownRef.current) {
      ...
      setTimeout(() => {
        ...
        toast({
          ...
          action: (
            <ToastAction altText={t.subscribeAction} onClick={() => requestSubscribe()}>
              {t.subscribeAction}
            </ToastAction>
          ),
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (shouldPromptSubscribe) {
        // 关闭弹窗后尝试拉起微信原生订阅授权面板（无额外自定义弹窗）
        pendingSubscribeRef.current = true
        onClose()
        // next tick: ensure dialog unmounted, then trigger
        setTimeout(() => requestSubscribe(), 0)
      } else {
        onClose()
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      {/* Hidden wx-open-subscribe tag: used to trigger WeChat native subscribe panel */}
      {shouldPromptSubscribe && subscribeTemplateId && openTagReady
        ? React.createElement(
            ...
            React.createElement('script', {
              ...
              dangerouslySetInnerHTML: {
                __html: `<button>subscribe</button>`,
              },
            }),
          )
        : null}
