...
- H5「开启微信通知」入口：详情页「联系咨询」弹窗中“允许回复消息发送到我的微信”（默认勾选），提交留言成功后展示 `wx-open-subscribe`（按官方文档的 default/style 插槽模板）供用户点击完成订阅授权：`src/app/[locale]/m/components/MobileContactUsModal.tsx`
