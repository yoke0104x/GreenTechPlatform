...
- H5「开启微信通知」入口：详情页「联系咨询」弹窗中“允许回复消息发送到我的微信”（默认勾选），提交留言成功后直接使用官方开放标签 `wx-open-subscribe`（默认按钮模板/样式）完成订阅授权：`src/app/[locale]/m/components/MobileContactUsModal.tsx`
