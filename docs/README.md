...
- **地区 / 园区关联**
  ...
  - `park_id`：园区ID（园区政策关联园区，外键指向园区表 `park.id`，可为空）
- **时间元数据**
...
### 园区表说明（后续补充）
- 当前仅约定：
  - `park` 表中存在唯一主键 `id`，供 `policy.park_id` 做外键关联；
...
### 阶段 1：数据与迁移
- [x] 1.1 确认地区数据来源与复用方案（采用方案 A：直接复用现有地区相关表——地方政策的 `region_id` 先统一指向 `admin_provinces.id`，园区相关政策的 `park_id` 暂时指向现有 `admin_development_zones.id`，后续如需更丰富园区信息再通过新增园区表扩展）
...
- [x] 2.2 设计并实现“政策列表 + 筛选”接口  
      ...
      - 查询参数约定：  
        ...
        - `developmentZone`: 经开区编码或 ID（沿用现有 `developmentZone` 约定，内部映射到 `admin_development_zones.id` → `policy.park_id`）  
